...
// track session ID and phone call ID (typically stored in persistent storage)
let sessionId = false;
let callId = false;
...
/**
 * Bandwidth's Voice API hits this endpoint when an outgoing call is answered
 */
app.post("/callAnswered", async (req, res) => {
    ...
    if (!participant) {
        ...
        res.status(200).send(); // must return 200 to the Bandwidth server
        return;
    }
...
// ------------------------------------------------------------------------------------
// Interactions with Bandwidth's WebRTC Services
/**
 * 
 * @param session_id 
 */
 ...
 * This will either create one via the API,  or return the one already created
 * @param account_id 
 * @param tag 
 * @return a Session id 
 */
async function getSessionId(account_id, tag) {
    // check if we've already created a session for the call
    // for this app, we will just check if sessionId is defined
    if (sessionId) {
        return sessionId;
    }
 ...
 * @return list: (Participant json object, participant token)
 */
